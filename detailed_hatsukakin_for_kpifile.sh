...
# term=4
tdate=$(date -d"0 days ago $param_date" +%Y-%m-%d); sdate=$(date -d"$tdate" +%Y-%m-01);echo "$sdate"" "" ""$tdate"; array=($(echo "
...
#u.date = unix_timestamp('2016-1-1')
where
k.kakin_st = 3 and
k.log_type = 3 and
...
#u.date = unix_timestamp('2016-1-1') and
