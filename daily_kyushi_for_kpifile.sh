...
抽出にはdaily_kyushi_aff_career_defice.shを利用
#COMMENT
#入力チェック
if [[ "$1" ]]
then
...
echo -n "" > $outfile
sh daily_kyushi_aff_career_device.sh $param_date $aff > $outfile
done
done
...
value=$(grep -e "^$param_date" output/daily_$1_1_$aff.kyushi.txt | cut -f2- | grep -e "^$str" | cut -f2)
if [ $value  ]
then
